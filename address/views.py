...
def checkout_new_address(request):
    if request.method == 'POST':
        ...
        addr = Address.objects.create(user=user,email=request.user.email,pin_code = pin_code,district=district,state=state,full_name=full_name,address=address,landmark=landmark,mobile_number=mobile_number)
        addr.save()
...
def checkout_edit_address(request):
    pk = request.POST['checkout_edit_address_pk'][11:]
    addr = Address.objects.get(pk=pk)
    full_name = request.POST['full_name']
    address = request.POST['address']
    landmark = request.POST['landmark']
